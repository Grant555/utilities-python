...
def get_first_url(html):
    if isinstance(html, str):
        return BeautifulSoup(html, 'lxml').find('article').header.h2.a['href']
...
def get_next_url(html):
    if isinstance(html, str):
        previous = BeautifulSoup(html, 'lxml').find('div', {'class': 'nav-previous'})
        if previous:
...
def save_article(html, thr_id):
    print('线程id: {}'.format(thr_id))
...
def worker(url):
    ...
    if isinstance(url, str):
        ...
        while url != '':
            ...
            thr = threading.Thread(target=save_article, args=(html, count))
            threads.append(thr)
            thr.start()
...
def main():
    ...
    print('任务完成！耗时：{}s'.format(end.total_seconds()))
